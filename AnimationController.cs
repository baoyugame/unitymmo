...
public class AnimationController : MonoBehaviour
{
	...
	// Verify setup, configure
	{
		...
		if (VerifySetup ())
		{
			...
			controller.onAttack += onAttack;
				// Have OnAttack invoked when the ThirdPersonController starts a attack
	...
	void onAttack(string type)
	//Start a attack
	{
		canLand = false;
		state = CharacterState.Attack;
		string attackAction = "attack" + type;
		target.CrossFade (attackAction);
		
		Invoke ("stopAttack", target[attackAction].length);
	}
	...
	// Start a landing
	{
		...
		Invoke (
			"Land",
			target["idle"].length * (HorizontalMovement.magnitude < walkSpeed ? 1.0f : runningLandingFactor)
